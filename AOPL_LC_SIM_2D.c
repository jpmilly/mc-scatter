...
int __declspec(dllexport) APIENTRY UserBulkDefinition(double *data)
{
	...
	offset = data[54]; //offset for absorption coefficient
	power = data[55];
...
get_new_random_number:;
	...
	random_number = dist(gen);
	alpha_exp = ((wave - absCenter)*(wave - absCenter)) / (abs1e*abs1e);
	
	alpha = exp(-alpha_exp)+offset*(1 / (pow(wave, power)));
...
get_new_random_number2:;
	...
	angle = 2.0*PI*random_number;

	//return scattered ray cosines to ZEMAX
	data[35] = cos(angle);
	data[36] = cos((PI / 2) - angle);
	data[37] = 0.0;
...
int __declspec(dllexport) APIENTRY UserParamNames(char *data)
{
	...
	i = (int)data[0];
	strcpy(data, "");
	if (i == 1) strcpy(data, "Abs. Center");
	if (i == 2) strcpy(data, "Abs. 1/e Width");
	if (i == 3) strcpy(data, "PLQY");
	if (i == 4) strcpy(data, "Abs. Offset");
	if (i == 5) strcpy(data, "Tail Power");
	return 0;
}
